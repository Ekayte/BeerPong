...
void loop() {
  ...
  switch (state) {
    ...
    case TRIANGLE_GAME_1:
       ...
       if(digitalRead(PIR_PIN2) == HIGH || digitalRead(PIR_PIN3) == HIGH || digitalRead(PIR_PIN5) == HIGH|| digitalRead(PIR_PIN6) == HIGH || digitalRead(PIR_PIN7) == HIGH || digitalRead(PIR_PIN8) == HIGH || digitalRead(PIR_PIN9) == HIGH ){
        if(led4 == 1){
          for (int i = 0; i < NUMPIXELS; i++) {
          strip10.setPixelColor(i, strip10.Color(0, 255, 0));
          strip9.setPixelColor(i, strip9.Color(0, 255, 0));

...
int value_rphoto_1 = 0;
int value_rphoto_12 = 0;
int value_rdif_1 = VALUE_RESET; // est égale à la valeur de reset par défaut 
...
switch (state) {
    ...
    case CHECK_RESISTANCE:
      ...
      if(led4){
        value_rphoto_1 = analogRead(RPHOTO_1); //on récupère la valeur de la résisantce 
        if(value_rdif_1 < VALUE_RESET){ //sinon on reset la couleur au vert
          for (int i = 0; i < NUMPIXELS; i++) { 
    ...
    value_rphoto_12 = analogRead(RPHOTO_1); //on récupère la valeur de la résisantce 
